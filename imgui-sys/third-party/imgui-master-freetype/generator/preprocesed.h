...
struct ImVec2
{
    ...
    float& operator[] (size_t idx) { (__builtin_expect(!(idx == 0 || idx == 1), 0) ? __assert_rtn(__func__, "imgui.h", 278, "idx == 0 || idx == 1") : (void)0); return ((float*)(void*)(char*)this)[idx]; }
    float operator[] (size_t idx) const { (__builtin_expect(!(idx == 0 || idx == 1), 0) ? __assert_rtn(__func__, "imgui.h", 279, "idx == 0 || idx == 1") : (void)0); return ((const float*)(const void*)(const char*)this)[idx]; }
};
struct ImVec4
{
    float x, y, z, w;
...
struct ImVector
{
    ...
    inline T& operator[](int i) { (__builtin_expect(!(i >= 0 && i < Size), 0) ? __assert_rtn(__func__, "imgui.h", 1990, "i >= 0 && i < Size") : (void)0); return Data[i]; }
    inline const T& operator[](int i) const { (__builtin_expect(!(i >= 0 && i < Size), 0) ? __assert_rtn(__func__, "imgui.h", 1991, "i >= 0 && i < Size") : (void)0); return Data[i]; }
    inline T* begin() { return Data; }
    inline const T* begin() const { return Data; }
    inline T* end() { return Data + Size; }
    inline const T* end() const { return Data + Size; }
    inline T& front() { (__builtin_expect(!(Size > 0), 0) ? __assert_rtn(__func__, "imgui.h", 1997, "Size > 0") : (void)0); return Data[0]; }
    inline const T& front() const { (__builtin_expect(!(Size > 0), 0) ? __assert_rtn(__func__, "imgui.h", 1998, "Size > 0") : (void)0); return Data[0]; }
    inline T& back() { (__builtin_expect(!(Size > 0), 0) ? __assert_rtn(__func__, "imgui.h", 1999, "Size > 0") : (void)0); return Data[Size - 1]; }
    inline const T& back() const { (__builtin_expect(!(Size > 0), 0) ? __assert_rtn(__func__, "imgui.h", 2000, "Size > 0") : (void)0); return Data[Size - 1]; }
    ...
    inline void shrink(int new_size) { (__builtin_expect(!(new_size <= Size), 0) ? __assert_rtn(__func__, "imgui.h", 2006, "new_size <= Size") : (void)0); Size = new_size; }
    ...
    inline void pop_back() { (__builtin_expect(!(Size > 0), 0) ? __assert_rtn(__func__, "imgui.h", 2012, "Size > 0") : (void)0); Size--; }
    inline void push_front(const T& v) { if (Size == 0) push_back(v); else insert(Data, v); }
    inline T* erase(const T* it) { (__builtin_expect(!(it >= Data && it < Data + Size), 0) ? __assert_rtn(__func__, "imgui.h", 2014, "it >= Data && it < Data + Size") : (void)0); const ptrdiff_t off = it - Data; __builtin___memmove_chk (Data + off, Data + off + 1, ((size_t)Size - (size_t)off - 1) * sizeof(T), __builtin_object_size (Data + off, 0)); Size--; return Data + off; }
    inline T* erase(const T* it, const T* it_last){ (__builtin_expect(!(it >= Data && it < Data + Size && it_last >= it && it_last <= Data + Size), 0) ? __assert_rtn(__func__, "imgui.h", 2015, "it >= Data && it < Data + Size && it_last >= it && it_last <= Data + Size") : (void)0); const ptrdiff_t count = it_last - it; const ptrdiff_t off = it - Data; __builtin___memmove_chk (Data + off, Data + off + count, ((size_t)Size - (size_t)off - (size_t)count) * sizeof(T), __builtin_object_size (Data + off, 0)); Size -= (int)count; return Data + off; }
    inline T* erase_unsorted(const T* it) { (__builtin_expect(!(it >= Data && it < Data + Size), 0) ? __assert_rtn(__func__, "imgui.h", 2016, "it >= Data && it < Data + Size") : (void)0); const ptrdiff_t off = it - Data; if (it < Data + Size - 1) __builtin___memcpy_chk (Data + off, Data + Size - 1, sizeof(T), __builtin_object_size (Data + off, 0)); Size--; return Data + off; }
    inline T* insert(const T* it, const T& v) { (__builtin_expect(!(it >= Data && it <= Data + Size), 0) ? __assert_rtn(__func__, "imgui.h", 2017, "it >= Data && it <= Data + Size") : (void)0); const ptrdiff_t off = it - Data; if (Size == Capacity) reserve(_grow_capacity(Size + 1)); if (off < (int)Size) __builtin___memmove_chk (Data + off + 1, Data + off, ((size_t)Size - (size_t)off) * sizeof(T), __builtin_object_size (Data + off + 1, 0)); __builtin___memcpy_chk (&Data[off], &v, sizeof(v), __builtin_object_size (&Data[off], 0)); Size++; return Data + off; }
    ...
    inline int index_from_ptr(const T* it) const { (__builtin_expect(!(it >= Data && it < Data + Size), 0) ? __assert_rtn(__func__, "imgui.h", 2024, "it >= Data && it < Data + Size") : (void)0); const ptrdiff_t off = it - Data; return (int)off; }
};
struct ImGuiStyle
{
    float Alpha;
...
struct ImGuiTextBuffer
{
    ...
    ImGuiTextBuffer() { }
    inline char operator[](int i) const { (__builtin_expect(!(Buf.Data != ((void*)0)), 0) ? __assert_rtn(__func__, "imgui.h", 2443, "Buf.Data != ((void*)0)") : (void)0); return Buf.Data[i]; }
...
struct ImFontAtlas
{
    ...
    ImFontAtlasCustomRect* GetCustomRectByIndex(int index) { (__builtin_expect(!(index >= 0), 0) ? __assert_rtn(__func__, "imgui.h", 3082, "index >= 0") : (void)0); return &CustomRects[index]; }
